...
if ( ! function_exists( 'pelicula_core_add_page_title_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function pelicula_core_add_page_title_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'description' => esc_html__( 'Global Title Options', 'pelicula-core' )
			)
		);
		...
		if ( $page ) {
			...
			$page_title_section->add_field_element(
				array(
					...
					'options'       => apply_filters( 'pelicula_core_filter_title_layout_options', array() ),
			...
			$page_title_section->add_field_element(
				array(
					...
					'options'       => pelicula_core_get_select_type_options_pool( 'title_tag', false, array(), array('div' => esc_html__( 'Custom', 'pelicula-core' ) ) ),
					'default_value' => 'h1'
				)
			);
			...
			do_action( 'pelicula_core_action_after_page_title_options_map', $page_title_section );
		}
	}

	add_action( 'pelicula_core_action_default_options_init', 'pelicula_core_add_page_title_options', pelicula_core_get_admin_options_map_position( 'title' ) );
}