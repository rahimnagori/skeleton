...
if ( ! function_exists( 'pelicula_core_register_breadcrumbs_title_layout' ) ) {
	function pelicula_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'PeliculaCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'pelicula_core_filter_register_title_layouts', 'pelicula_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'pelicula_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'pelicula-core' );
		
		return $layouts;
	}
	
	add_filter( 'pelicula_core_filter_title_layout_options', 'pelicula_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'pelicula_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function pelicula_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'pelicula_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'pelicula-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'pelicula-core' ),
			'author'      => esc_html__( 'Posted by %s', 'pelicula-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'pelicula-core' ),
			'404'         => esc_html__( '404 - Page not found', 'pelicula-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'pelicula-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'pelicula_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'pelicula_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'pelicula_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}