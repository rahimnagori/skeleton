...
class PeliculaCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'pelicula_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
		
		// Add title area inline styles
		add_filter( 'pelicula_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( PELICULA_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( PELICULA_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = pelicula_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'pelicula_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'pelicula_filter_title_template' : 'pelicula_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = pelicula_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = pelicula_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = pelicula_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = pelicula_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile     	  	  = pelicula_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding    	  	  = apply_filters( 'pelicula_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'pelicula_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = pelicula_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = pelicula_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = pelicula_core_get_page_title_image_params();
		...
		$title_color = pelicula_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
PeliculaCoreTitles::get_instance();