...
if ( ! function_exists( 'pelicula_core_add_portfolio_archive_sidebar_options' ) ) {
	/**
	 * Function that add sidebar options for portfolio archive module
	 */
	function pelicula_core_add_portfolio_archive_sidebar_options( $tab ) {
		...
		if ( $tab ) {
			$tab->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose default sidebar layout for portfolio archives', 'pelicula-core' ),
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$tab->add_field_element(
					array(
						...
						'description'   => esc_html__( 'Choose a custom sidebar to display on portfolio archives', 'pelicula-core' ),
	...
	add_action( 'pelicula_core_action_after_portfolio_options_archive', 'pelicula_core_add_portfolio_archive_sidebar_options' );
}

if ( ! function_exists( 'pelicula_core_add_portfolio_single_sidebar_options' ) ) {
	/**
	 * Function that add sidebar options for portfolio single module
	 */
	function pelicula_core_add_portfolio_single_sidebar_options( $tab ) {
	...
	add_action( 'pelicula_core_action_after_portfolio_options_single', 'pelicula_core_add_portfolio_single_sidebar_options' );
}