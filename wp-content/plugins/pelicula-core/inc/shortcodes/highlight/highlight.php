...
if ( ! function_exists( 'pelicula_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_highlight_shortcode' );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreHighlightShortcode extends PeliculaCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'pelicula_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'pelicula-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_tag',
				'title'         => esc_html__( 'Text Tag', 'pelicula-core' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable Text Line Break', 'pelicula-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'pelicula-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'pelicula-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'pelicula-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
