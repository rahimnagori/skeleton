...
if ( ! function_exists( 'pelicula_core_add_pricing_table_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_pricing_table_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCorePricingTableShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_pricing_table_shortcode' );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCorePricingTableShortcode extends PeliculaCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'pelicula_core_filter_pricing_table_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'pelicula_core_filter_pricing_table_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/pricing-table' );
			$this->set_base( 'pelicula_core_pricing_table' );
			$this->set_name( esc_html__( 'Pricing Table', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds pricing table element', 'pelicula-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'featured_table',
				'title'         => esc_html__( 'Featured Table', 'pelicula-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'price',
				'title'      => esc_html__( 'Price', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'price_color',
				'title'      => esc_html__( 'Price Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'currency',
				'title'      => esc_html__( 'Currency', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'currency_color',
				'title'      => esc_html__( 'Currency Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'html',
				'name'       => 'content',
				'title'      => esc_html__( 'Content', 'pelicula-core' )
			) );
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/pricing-table', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
