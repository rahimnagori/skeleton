...
abstract class PeliculaCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'pelicula_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );
	...
	public function map_list_options( $params = array() ) {
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behaviour', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'options'       => pelicula_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Image Proportions', 'pelicula-core' ),
				'options'       => array(
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'custom_proportions',
				'title'         => esc_html__( 'Custom Proportions', 'pelicula-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Image Proportions', 'pelicula-core' ),
				'options'       => pelicula_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'pelicula-core' ),
				'description' => esc_html__( 'Enter image width in px', 'pelicula-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'pelicula-core' ),
				'description' => esc_html__( 'Enter image height in px', 'pelicula-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'options'       => pelicula_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
				'default_value' => '3',
				'group'         => $group,
				'dependency'    => array(
					'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'pelicula-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'pelicula-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'pelicula-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'pelicula-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
        $this->set_option(
            array(
                ...
                'title'      => esc_html__( 'Choose Slide Animation', 'pelicula-core' ),
                'options'    => array(
                    ''     => esc_html__( 'Default (Slide)', 'pelicula-core' ),
                    'fade' => esc_html__( 'Fade', 'pelicula-core' ),
                ),
                'dependency' => $dependency,
                'group'      => $group,
            )
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Slider Navigation Type', 'pelicula-core' ),
			'options'    => array(
				''       => esc_html__( 'Default', 'pelicula-core' ),
				'custom' => esc_html__( 'Bottom Side-aligned', 'pelicula-core' )
			),
			'dependency' => array(
				'show' => array(
	...
	public function get_list_classes( $atts ) {
		...
		$holder_classes[] = ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' && ! empty( $atts['custom_proportions'] ) && $atts['custom_proportions'] == 'yes' ? 'qodef-items-custom-proportions--enabled' : '';
		...
		if ( isset( $atts['slider_layout'] ) && $atts['slider_layout'] === 'predefined' && $atts['enable_fs_predefined_slider'] === 'yes' ) {
			$holder_classes[] = 'qodef-slider-layout--predefined-fullscreen';
		}
		...
		if ( ! empty( $atts['slider_navigation_type'] ) ) {
			$holder_classes[] = 'qodef-nav--' . $atts['slider_navigation_type'];
		}
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = pelicula_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
		
		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ) ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => pelicula_core_get_custom_image_size_class_name( $atts['images_proportion'] )
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? pelicula_core_get_space_value( $atts['space'] ) : 0;
		...
		if ( isset( $atts['slider_layout'] ) && $atts['slider_layout'] === 'predefined2' ) {
			$data['slidesPerView']  = 1.9;
			$data['centeredSlides'] = true;
			$data['slideToClickedItem'] = true;
		}
		
		if ( isset( $atts['slider_layout'] ) && $atts['slider_layout'] === 'predefined3' ) {
			$data['slidesPerView']  = 4;
			$data['centeredSlides'] = true;
			$data['slideToClickedItem'] = true;
			$data['roundLengths'] = true;
			//			$data['customStages']      = true;
			//			$data['slidesPerView1366'] = 2;
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'pelicula-core' );
		...
		$this->set_option( array(
			'field_type'    => 'text',
			'name'          => 'posts_per_page',
			'title'         => esc_html__( 'Posts per Page', 'pelicula-core' ),
			'default_value' => '-1',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'pelicula-core' ),
			'options'       => pelicula_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'order',
			'title'         => esc_html__( 'Order', 'pelicula-core' ),
			'options'       => pelicula_core_get_select_type_options_pool( 'order' ),
			'default_value' => 'DESC',
			'group'         => $group
		) );
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option ) ) ) {
			...
			do_action( 'pelicula_core_action_map_query_options_before_additional', $group );
			
			$additional_params = apply_filters( 'pelicula_core_filter_map_additional_query_params', array(
				''       => esc_html__( 'No', 'pelicula-core' ),
				'id'     => esc_html__( 'Post IDs', 'pelicula-core' ),
				'tax'    => esc_html__( 'Taxonomy Slug', 'pelicula-core' ),
				'author' => esc_html__( 'Author Name', 'pelicula-core' ),
			) );
			
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'additional_params',
				'title'      => esc_html__( 'Additional Params', 'pelicula-core' ),
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'post_ids',
				'title'       => esc_html__( 'Posts IDs', 'pelicula-core' ),
				'description' => esc_html__( 'Separate post IDs with commas', 'pelicula-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option( array(
					'field_type' => 'select',
					'name'       => 'tax',
					'title'      => esc_html__( 'Taxonomy', 'pelicula-core' ),
				...
				$this->set_option( array(
					'field_type' => 'text',
					'name'       => 'tax_slug',
					'title'      => esc_html__( 'Taxonomy Slug', 'pelicula-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
				...
				$this->set_option( array(
					'field_type'  => 'text',
					'name'        => 'tax__in',
					'title'       => esc_html__( 'Taxonomy IDs', 'pelicula-core' ),
					'description' => esc_html__( 'Separate taxonomy IDs with commas', 'pelicula-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
							'additional_params' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'author_slug',
				'title'      => esc_html__( 'Author Slug', 'pelicula-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			do_action( 'pelicula_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'pelicula_core_filter_additional_query_args', $args, $atts );
	...
	public function map_layout_options( $params = array() ) {
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'pelicula-core' ),
	...
	public function map_additional_options( $params = array() ) {
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'pelicula-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'pelicula-core' ),
				...
				'group'         => esc_html__( 'Additional', 'pelicula-core' )
			) );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'pelicula-core' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'pelicula-core' ),
				'dependency'    => array(
					'show' => array(
	...
	public function load_assets() {
		do_action( 'pelicula_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
