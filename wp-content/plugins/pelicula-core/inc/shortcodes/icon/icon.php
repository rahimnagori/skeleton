...
if ( ! function_exists( 'pelicula_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreIconShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreIconShortcode extends PeliculaCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'pelicula_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'pelicula-core' ) );
			...
			$this->set_option( array(
				...
				'options'    => array(
					...
					'tiny'    => esc_html__( 'Tiny', 'pelicula-core' ),
					'small'   => esc_html__( 'Small', 'pelicula-core' ),
					'medium'  => esc_html__( 'Medium', 'pelicula-core' ),
					'large'   => esc_html__( 'Large', 'pelicula-core' ),
					'huge'    => esc_html__( 'Huge', 'pelicula-core' ),
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'custom_size',
				'title'      => esc_html__( 'Custom Size', 'pelicula-core' ),
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'icon_layout',
				...
				'options'       => array(
					'normal' => esc_html__( 'Normal', 'pelicula-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_radius',
				'title'      => esc_html__( 'Border Radius', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => 'square',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'shape_size',
				'title'      => esc_html__( 'Shape Size', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
			...
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'color',
				'title'      => esc_html__( 'Icon Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'hover_color',
				'title'      => esc_html__( 'Icon Hover Color', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_width',
				'title'      => esc_html__( 'Border Width', 'pelicula-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => array( 'circle', 'square' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'pelicula_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
