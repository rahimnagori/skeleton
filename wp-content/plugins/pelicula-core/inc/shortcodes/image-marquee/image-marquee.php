...
if ( ! function_exists( 'pelicula_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreImageMarqueeShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreImageMarqueeShortcode extends PeliculaCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'pelicula_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'pelicula_core_filter_image_marquee_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'pelicula_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'pelicula-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'duration',
				'title'         => esc_html__( 'Animation Duration (Seconds)', 'pelicula-core' ),
				'default_value' => '20',
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
