...
if ( ! function_exists( 'pelicula_core_add_vertical_split_slider_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_vertical_split_slider_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaVerticalSplitSliderShortcode';

		return $shortcodes;
	}

	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_vertical_split_slider_shortcode' );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaVerticalSplitSliderShortcode extends PeliculaCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider' );
			$this->set_base( 'pelicula_vertical_split_slider' );
			$this->set_name( esc_html__( 'Vertical Split Slider', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds vertical split slider holder', 'pelicula-core' ) );
			...
			$this->set_scripts(
				array(
					...
					'multiscroll' => array(
						'registered'	=> false,
						'url'			=> PELICULA_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider/assets/js/plugins/jquery.multiscroll.min.js',
			...
			$this->set_necessary_styles(
				array(
					'multiscroll' => array(
						'registered'	=> false,
						'url'			=> PELICULA_CORE_SHORTCODES_URL_PATH . '/vertical-split-slider/assets/css/plugins/jquery.multiscroll.css'
					)
				)
			);
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable on smaller screens', 'pelicula-core' ),
				'options'       => array(
					'1024' => esc_html__( 'Below 1024px', 'pelicula-core' ),
					'768'  => esc_html__( 'Below 768px', 'pelicula-core' ),
				),
				'default_value' => '1024'
			) );
			$this->set_option( array(
				'field_type' => 'repeater',
				'name'       => 'children',
				'title'      => esc_html__( 'Slide Items', 'pelicula-core' ),
				'items'      => array(
					array(
						'field_type' => 'select',
						'name'       => 'slide_header_style',
						'title'      => esc_html__( 'Header/Bullets Style', 'pelicula-core' ),
						'options'    => array(
					...
					array(
						'field_type' => 'select',
						'name'       => 'slide_layout',
						'title'      => esc_html__( 'Slide Layout', 'pelicula-core' ),
						'options'    => array(
							'image-left'  => esc_html__( 'Image On Left', 'pelicula-core' ),
							'image-right' => esc_html__( 'Image On Right', 'pelicula-core' )
						),
					),
					array(
						'field_type' => 'image',
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/vertical-split-slider', 'templates/vertical-split-slider', '', $atts );
		}
