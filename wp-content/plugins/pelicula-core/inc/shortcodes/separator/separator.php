...
if ( ! function_exists( 'pelicula_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreSeparatorShortcode extends PeliculaCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'pelicula_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'pelicula-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'pelicula-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'pelicula-core' ),
				'options'    => pelicula_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
