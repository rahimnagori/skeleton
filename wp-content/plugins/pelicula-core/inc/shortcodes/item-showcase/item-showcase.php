...
if ( ! function_exists( 'pelicula_core_add_item_showcase_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_item_showcase_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreItemShowcaseShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_item_showcase_shortcode' );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreItemShowcaseShortcode extends PeliculaCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'pelicula_core_filter_item_showcase_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/item-showcase' );
			$this->set_base( 'pelicula_core_item_showcase' );
			$this->set_name( esc_html__( 'Item Showcase', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds item showcase holder', 'pelicula-core' ) );
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Image Top Offset', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'repeater',
				'name'       => 'children',
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/item-showcase', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
