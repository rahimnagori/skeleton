...
if ( ! function_exists( 'pelicula_core_add_button_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_button_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreButtonShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_button_shortcode', 9 );
}

if ( class_exists( 'PeliculaCoreShortcode' ) ) {
	class PeliculaCoreButtonShortcode extends PeliculaCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'pelicula_core_filter_button_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/button' );
			$this->set_base( 'pelicula_core_button' );
			$this->set_name( esc_html__( 'Button', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays button with provided parameters', 'pelicula-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'size',
				'title'      => esc_html__( 'Size', 'pelicula-core' ),
				'options'    => array(
					''      => esc_html__( 'Normal', 'pelicula-core' ),
					'small' => esc_html__( 'Small', 'pelicula-core' ),
					'large' => esc_html__( 'Large', 'pelicula-core' )
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'target',
				'title'         => esc_html__( 'Target', 'pelicula-core' ),
			...
			$this->set_option( array(
				...
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'name'       => 'hover_color',
				'field_type' => 'color',
				'title'      => esc_html__( 'Text Hover Color', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Background Hover Color', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'border_color',
				'title'      => esc_html__( 'Border Color', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'hover_border_color',
				'title'      => esc_html__( 'Border Hover Color', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin',
				'title'      => esc_html__( 'Margin', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'padding',
				'title'      => esc_html__( 'Padding', 'pelicula-core' ),
				'group'      => esc_html__( 'Style', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				...
				'group'      => esc_html__( 'Typography', 'pelicula-core' )
			) );
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'font_weight',
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'html_type',
				'title'         => esc_html__( 'HTML Type', 'pelicula-core' ),
				'options'       => array(
					'default' => esc_html__( 'Default', 'pelicula-core' ),
					'input'   => esc_html__( 'Input', 'pelicula-core' ),
					'submit'  => esc_html__( 'Submit', 'pelicula-core' )
				),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'input_name',
				'title'      => esc_html__( 'Input Name', 'pelicula-core' ),
				'visibility'    => array(
					'map_for_page_builder'    => false
				)
			) );
			$this->set_option( array(
				'field_type' => 'array',
				'name'       => 'custom_attrs',
				'title'      => esc_html__( 'Custom Data Attributes', 'pelicula-core' ),
				'visibility'    => array(
					'map_for_page_builder'    => false
				)
			) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'pelicula_core_button', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/button', 'variations/'.$atts['button_layout'].'/templates/' . $atts['html_type'], '', $atts );
		}
		...
		private function get_data_attrs( $atts ) {
			...
			if ( ! empty( $atts['background_color'] ) ) {
				$data['data-background-color'] = $atts['background_color'];
			}
