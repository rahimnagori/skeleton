...
if ( ! function_exists( 'pelicula_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pelicula_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'PeliculaCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pelicula_core_filter_register_shortcodes', 'pelicula_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'PeliculaCoreListShortcode' ) ) {
	class PeliculaCoreImageGalleryShortcode extends PeliculaCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PELICULA_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'pelicula_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'pelicula-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'pelicula-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'pelicula-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'pelicula-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'pelicula_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return pelicula_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
