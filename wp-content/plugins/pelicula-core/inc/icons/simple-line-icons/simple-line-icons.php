...
if ( ! function_exists( 'pelicula_core_add_simple_line_icons_to_collection' ) ) {
	function pelicula_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'PeliculaCoreSimpleLineIconsPack';
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'pelicula_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class PeliculaCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return PELICULA_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
