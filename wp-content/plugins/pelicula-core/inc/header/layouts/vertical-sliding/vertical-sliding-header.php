...
class VerticalSlidingHeader extends PeliculaCoreHeader {
	...
	public function set_inline_header_styles( $style ) {
		...
		if ( ! empty( $border_color ) ) {
			$border_styles['border-right'] = '2px solid ' . $border_color;
		}

		if ( ! empty( $border_styles ) ) {
			$style .= qode_framework_dynamic_style( '.qodef-header--' . $this->slug . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--static', $border_styles );
			$style .= qode_framework_dynamic_style( '.qodef-header--' . $this->slug . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--dynamic', $border_styles );
		}

		$inner_styles = array();

		$header_padding_top = pelicula_core_get_post_value_through_levels( 'qodef_vertical_sliding_header_padding_top' );
		...
		$header_padding_bottom = pelicula_core_get_post_value_through_levels( 'qodef_vertical_sliding_header_padding_bottom' );
		...
		if ( ! empty( $inner_styles ) ) {
			$style .= qode_framework_dynamic_style( '.qodef-header--' . $this->slug . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--static', $inner_styles );
			$style .= qode_framework_dynamic_style( '.qodef-header--' . $this->slug . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--dynamic', $inner_styles );
		}
